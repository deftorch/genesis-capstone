/**
 * Default system instructions for the Genesis chat/generation API.
 *
 * This is extracted into its own module (instead of being an inline string in
 * app/api/chat/route.ts) so it can be:
 *  - reused/tested independently
 *  - displayed in the Settings UI (Developer tab) for transparency
 *  - overridden by a user-provided custom prompt at request time
 */
export const DEFAULT_SYSTEM_PROMPT = `You are Genesis, a creative AI assistant specialized in generating visual content using p5.js, D3.js, SVG, Mermaid.js, Two.js, Mo.js, and Remotion.
...
const RENDERER_NAME_BY_MARKER: Record<string, string> = {
  d3: 'D3.js',
  svg: 'SVG',
  twojs: 'Two.js',
  mojs: 'Mo.js',
...
function detectRendererName(trimmedCode: string): string {
  const match = trimmedCode.match(/^\/\/\s*renderer:\s*(\w+)/i);
  const marker = match?.[1]?.toLowerCase();
  return (marker && RENDERER_NAME_BY_MARKER[marker]) || 'p5.js';
}

/**
 * Builds the final system prompt sent to Gemini for a given request.
 *
 * @param basePrompt The base system prompt (persona + renderer rules). Defaults
 *                    to DEFAULT_SYSTEM_PROMPT but may be a user-supplied override
 *                    coming from Settings > Developer > System Instructions.
 * @param currentCode Sanitized current artifact code, if the user is editing an
 *                     existing artifact rather than starting from scratch.
 */
export function buildChatSystemPrompt(basePrompt: string, currentCode: string): string {
  let systemPrompt = basePrompt;

  if (currentCode && currentCode.trim()) {
    const trimmedCode = currentCode.trimStart();
    const rendererName = detectRendererName(trimmedCode);
    systemPrompt += `
...
`;
    ...
    systemPrompt += EXISTING_CODE_EXAMPLES;
  }
